...
namespace ProductionCode.Orders.Models
{
    public class Order
    {
        ...
        /// </summary>
        public decimal TotalRefunds => Refunds.Select(r => r.Amount).DefaultIfEmpty(0M).Sum();
        public List<Adjustment> Adjustments { get; } = new List<Adjustment>();
        public decimal TotalAdjustments => Adjustments.Select(a => a.Amount).DefaultIfEmpty(0M).Sum();
        ...
        public bool ShouldBeIgnored
        {
            get
            {
                ...
                if (!BookedDate.HasValue || BookedDate.Value < new DateTime(2016, 4, 1))
                    return true;
